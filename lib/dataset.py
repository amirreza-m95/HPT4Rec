...
class Dataset(object):
    def __init__(self, path, sep=',', session_key='SessionID', item_key='ItemID', time_key='Time', n_sample=-1, itemmap=None, itemstamp=None, time_sort=False):
        ...
        if n_sample > 0: #n_sample default is -1 , we can indicate number of samples to be used
        ...
        self.click_offsets = self.get_click_offset() #offset means deviation so [0,4,10,13,...] number of each session clicks
        self.session_idx_arr = self.order_session_idx() # array of number of unique sessionID
    ...
    def add_item_indices(self, itemmap=None):
        ...
        """
        if itemmap is None:
            item_ids = self.df[self.item_key].unique()  # type is numpy.ndarray , to get the unique values of the column
        ...
        self.df = pd.merge(self.df, self.itemmap, on=self.item_key, how='inner') # Merge df or series object with a database-style join.
    ...
    def get_click_offset(self):
        ...
        offsets[1:] = self.df.groupby(self.session_key).size().cumsum() #cumulatively sum the number of clicks on sessions 
        return offsets
